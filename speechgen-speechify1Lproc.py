...
# Get the API key for Speechify from environment variables
...
def fetch_speechify_voices():
    """Retrieve all Simba-English voices from Speechify API and return a list."""
            ...
            for model in voice["models"]:
                if model["name"] == "simba-english":
                    simba_english_voices.append({
                        ...
                        "gender": voice["gender"],
                        "region": model["languages"][0]["locale"] if model["languages"] else "Unknown"
                    })
        return simba_english_voices
    else:
        print(f"Failed to retrieve voices: {response.status_code}, {response.text}")
        return None

def get_voice_id_from_number(voice_number, voices):
    """Match a numerical voice number to an available Speechify voice."""
    if 1 <= voice_number <= len(voices):
        return voices[voice_number - 1]  # Return full voice dict
    return None
...
def process_csv(csv_filename):
    ...
    # Fetch Speechify voices and map the number to an ID
    available_voices = fetch_speechify_voices()
    if not available_voices:
        print("Error: No voices retrieved from Speechify API.")
        return
    ...
    speechify_voice_id = voice_data["id"]
    voice_name = voice_data["name"]

    # Debugging: Save and print the matched voice ID
    print(f"Mapped voice number {voice_number} to Speechify voice ID: {speechify_voice_id} ({voice_name})")

    try:
        timestamp_obj = datetime.strptime(timestamp_raw, "%m/%d/%Y %H:%M:%S")
        timestamp_formatted = timestamp_obj.strftime("%m%d%y-%H%M")  # Match original Speechify format
    except ValueError:
    ...
    # Run Speechify API with the extracted voice ID
    audio_filename = generate_speech(text_to_speak, speechify_voice_id, voice_name, timestamp_formatted, folder_name)
    if audio_filename:
...
def generate_speech(text, voice_id, voice_name, timestamp, folder):
    """Generate speech using Speechify API and save using original Speechify naming format."""
    import os

    # Construct filename and path
    filename = os.path.join(folder, f"speechify-{timestamp}-{voice_name}.mp3")
...
def send_email_with_applescript(folder_path, audio_filename):
    ...
    if emailaddy does not contain "Error" then
        ...
        tell application "Mail"
            set newMessage to make new outgoing message with properties {{subject:"Speechify Output - " & formattedDate, content:greeting & return & return & "Here is the speech file generated from your text input." & return & "Please review and let me know if any changes are needed." & return & return & "Best regards," & return & "Dr. Rose", visible:true}}
            tell newMessage
