...
def main():
    ...
    generate_pdf(latex_file, filename, filenameproc)
...
def generate_pdf(latex_file, filename, filenameproc):
    """
    ...
    pdf_output_filename = os.path.join(output_dir, "analysis.pdf")
    ...
    if subprocess.run(["command", "-v", "pdflatex"], capture_output=True).returncode == 0:
        ...
        shutil.move(filenameproc, os.path.join(output_dir, os.path.basename(filenameproc)))
