...
def process_files(working_directory, csv_file):
    ...
    for i in os.listdir(working_directory):
        ...
        if os.path.isfile(i_path) and get_file_size(i_path) > 25:
            ...
            # Insert the additional text before sending the content
            content_with_prompt = "Can you check the following paragraph, located after the colon, for grammar mistakes, count the number of mistakes, place the number of grammar mistakes between TWO @ symbols even if the number is zero, explain each of the mistakes, and provide a revised paragraph: " + content

            # Process the content with the first prompt
            first_output = send_prompt_and_capture_output(child, content_with_prompt)
