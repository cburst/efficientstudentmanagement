...
for i in range(num_rows):
    ...
    for j in range(total_rows):
        ...
        if similarity >= 0.90:
            R_matches.append(str((j % num_rows) + 2))  # Adjust for 1-based index, and account for header
            R_match_pct.append(similarity_pct)
        elif 0.70 <= similarity < 0.90:
            S_matches.append(str((j % num_rows) + 2))  # Adjust for 1-based index, and account for header
