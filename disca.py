...
def main():
    ...
    source_tsv = os.path.join("input-files", f"{dirname}raw.tsv")
    dest_tsv = os.path.join("folders", "gpt-disca", f"{dirname}raw.tsv")
    ...
    os.chdir(os.path.join("folders", "gpt-disca"))
    ...
    source_csv = os.path.join("folders", "gpt-disca", f"{dirname}.csv")
    dest_csv = os.path.join("output-files", f"{dirname}-DISCA-processed.csv")
